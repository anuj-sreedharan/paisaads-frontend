...
export const columns: ColumnDef<PosterAd>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <Zoom>
        <img
          src={`/api/images?imageName=${row.original.image?.fileName}`}
          alt={row.original.image?.fileName}
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return <PaymentDialog adId={row.original.id} type={AdType.POSTER} />;
    },
    sortingFn: (rowA, rowB) => {
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="flex gap-2">
        <Link href={`/dashboard/view-ad/poster-ad/${row.original.id}`}>
          <Button size="icon" variant="ghost">
            <Eye className="h-4 w-4" />
          </Button>
        </Link>
        <Link href={`/dashboard/edit-ad/poster-ad/${row.original.id}`}>
          <Button size="icon" variant="ghost">
