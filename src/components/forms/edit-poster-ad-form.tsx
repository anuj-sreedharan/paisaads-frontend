...
interface EditPosterAdFormProps {
  adId: string;
}

export default function EditPosterAdForm({ adId }: EditPosterAdFormProps) {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await api.get<AdComment[]>(
        `ad-comments/ad/${AdType.POSTER}/${adId}?actionType=${AdStatus.HOLD}`
      );
  ...
  const { mutate: updateAd, isPending: isSubmitting } = useMutation({
    mutationFn: async (data: PosterAdFormValues) => {
      const response = await api.patch(`/poster-ad/${adId}`, data);
      return response.data;
    },
    onSuccess: () => {
      ...
      router.push("/dashboard/my-ads/poster-ads");
    },
    onError: () => {
  ...
  const { mutate: sendForReview, isPending: isSendingForReview } = useMutation({
    mutationFn: async () => {
      const response = await api.post(
        `ad-comments/send-for-review/${AdType.POSTER}/${adId}`
      );
      return response.data;
    },
    onSuccess: () => {
  ...
  const form = useForm<PosterAdFormValues>({
    resolver: zodResolver(posterAdFormSchema),
    defaultValues: {
      dates: [],
  ...
  useEffect(() => {
    if (isLoading === false && ad !== undefined && profile !== undefined) {
      ...
      const formValues: Partial<PosterAdFormValues> = {
        imageId: ad.image.id,
  ...
  const onSubmit = (data: PosterAdFormValues) => {
                ...
                <FormItem>
                  <FormControl>
                    <div className="space-y-1">
                      ...
                      <Zoom>
                        <div className="relative aspect-[4/3] border border-dashed rounded overflow-hidden bg-gray-50">
                          {uploadedImage ? (
                            <>
                              <img
