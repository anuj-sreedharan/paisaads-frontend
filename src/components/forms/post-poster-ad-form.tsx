...
import { posterAdFormSchema, type PosterAdFormValues } from "@/lib/validations";
...
export function PostPosterAdForm() {
  ...
  const { mutate: submitAd, isPending: isSubmitting } = useMutation({
    mutationFn: async (data: PosterAdFormValues) => {
      const response = await api.post("/poster-ad", data);
      return response.data;
    },
    onSuccess: () => {
      toast.success("Poster Ad created successfully", {
        description:
    ...
    <div className="max-w-4xl mx-auto bg-white">
      <div className="p-6">
        <h1 className="text-2xl font-medium mb-5">
          Create a new Poster Advertisement
        </h1>
                    ...
                    <FormItem>
                      <FormControl>
                        <div className=" gap-4">
                          <div
                            className={cn(
                              "relative border border-gray-300 border-dashed rounded-md overflow-hidden bg-gray-50",
                              form.watch("side") === PositionType.CENTER_TOP ||
                              form.watch("side") === PositionType.CENTER_BOTTOM
                                ? "h-36 w-full"
                                : "h-96 w-56"
                            )}
                          >
